...
contract AssetTokenERC721 is ERC721, Ownable, IFixDescriptor {
    ...
    function getHumanReadableDescriptor() external view override returns (string memory) {
        require(_descriptorInitialized, "Descriptor not initialized");
        require(_collectionDescriptor.dictionaryContract != address(0), "Dictionary not set");
        require(_collectionDescriptor.fixCBORPtr != address(0), "CBOR not deployed");
        ...
        FixDictionary dictionary = FixDictionary(_collectionDescriptor.dictionaryContract);
